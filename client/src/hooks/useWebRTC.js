...
console.warn('Skipping ICE candidate: call ended or missing callId', { callId });
return;
}
if (event.candidate) {
if (!isMounted.current) {
console.log('Buffering ICE candidate due to unmounted component for call:', callId);
...
console.log('ICE candidate gathering complete for call:', callId);
}
};
pc.ontrack = (event) => {
...
console.log('Setting active remote stream for call:', callId);
setRemoteStream(stream);
} else {
console.warn('Received inactive remote stream for call:', callId);
}
};
pc.oniceconnectionstatechange = () => {
if (!isMounted.current) return;
const state = pc.iceConnectionState;
console.log('ICE connection state:', state, 'for call:', callId || 'unknown');
if (state === 'failed' || state === 'disconnected') {
setCallStatus('Connection issues detected. Attempting reconnect...');
setTimeout(() => {
if (pc.iceConnectionState !== 'connected' && pc.iceConnectionState !== 'completed') {
console.log('Reconnect timeout: Ending call:', callId);
endCall();
}
}, 5000);
...
console.log('Signaling state:', state, 'for call:', callId || 'unknown');
if (state === 'closed' && !isCleaningUp.current) {
console.log('Signaling closed: Ending call:', callId || 'unknown');
endCall();
}
};
return pc;
...
peerConnectionRef.current = createPeerConnection(newCallId);
